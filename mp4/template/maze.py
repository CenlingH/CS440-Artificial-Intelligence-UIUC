...
class Maze:
    ...
    def __init__(self, path, legend={'wall': '%', 'start': 'P', 'waypoint': '.'}):
        ...
        if any(len(line) != m for line in lines):
            raise MazeError(
                '(maze \'{0}\'): all maze rows must be the same length (shortest row has length {1})'.format(path, m))
        ...
        if any(self[x] != self.legend.wall for x in chain(
            ...
            raise MazeError('(maze \'{0}\'): maze borders must only contain `wall` cells (\'{1}\')'.format(
                path, self.legend.wall))
        if n < 3 or m < 3:
            raise MazeError(
                '(maze \'{0}\'): maze dimensions ({1}, {2}) must be at least (3, 3)'.format(path, n, m))
...
{{
    ...
    def draw_square(self, i, j, color=(0, 0, 0)):
        pygame.draw.rect(self.surface, color, tuple(
            i * self.scale for i in (j, i, 1, 1)), 0)

    def draw_circle(self, i, j, color=(0, 0, 0), radius=None):
        if radius is None:
            radius = self.scale / 4
        pygame.draw.circle(self.surface, color, tuple(
            int((i + 0.5) * self.scale) for i in (j, i)), int(radius))
    ...
    def draw_start(self):
        i, j = self.maze.start
        pygame.draw.rect(self.surface, (0, 0, 255), tuple(
            int(i * self.scale) for i in (j + 0.25, i + 0.25, 0.5, 0.5)), 0)
