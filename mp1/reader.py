...
from nltk.stem.porter import PorterStemmer  # 提取
from nltk.tokenize import RegexpTokenizer  # 分割
...
def load_dataset(train_dir, dev_dir, stemming=False, lower_case=False, silently=True):
    ...
    X0 = loadDir(train_dir + '/pos/', stemming,
                 lower_case, silently)  # training data
    X1 = loadDir(train_dir + '/neg/', stemming, lower_case, silently)
    X = X0 + X1  # 表示文字拼接的
    Y = len(X0) * [1] + len(X1) * [0]  # 是1和0的拼接

    X_test0 = loadDir(dev_dir + '/pos/', stemming,
                      lower_case, silently)  # development data
