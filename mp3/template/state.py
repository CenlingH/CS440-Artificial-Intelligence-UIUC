...
class WordLadderState(AbstractState):
    def __init__(self, state, goal, dist_from_start, use_heuristic):
        '''
        state: string of length n, 当前的单词
    ...
    def get_neighbors(self):
        ...
        for word_idx in range(len(self.state)):
            prefix = self.state[:word_idx]  # 从开头到word_idx，不包括word_idx
            suffix = self.state[word_idx+1:]  # 后面的
            # 'a' = 97, 'z' = 97 + 25 = 122
...
class EightPuzzleState(AbstractState):
    ...
    # 就是，动了一步之后可能造成的所有结果（最多4个），作为邻居，其实就是空格有四种动法
    def get_neighbors(self):
        '''
        ...
        if zero[0] != 2:
            ...
            next_state[zero[0]][zero[1]] = next_state[zero[0]+1][zero[1]]
            next_state[zero[0]+1][zero[1]] = 0
            neighbor = EightPuzzleState(next_state, self.goal,
                                        dist_from_start=self.dist_from_start + 1, use_heuristic=self.use_heuristic, zero_loc=(zero[0]+1, zero[1]))
            nbr_states.append(neighbor)
        if zero[1] != 0:
            next_state = copy.deepcopy(self.state)
            next_state[zero[0]][zero[1]] = next_state[zero[0]][zero[1]-1]
            next_state[zero[0]][zero[1]-1] = 0
            neighbor = EightPuzzleState(next_state, self.goal,
                                        dist_from_start=self.dist_from_start + 1, use_heuristic=self.use_heuristic, zero_loc=(zero[0], zero[1]-1))
            nbr_states.append(neighbor)
        if zero[0] != 0:
            next_state = copy.deepcopy(self.state)
            next_state[zero[0]][zero[1]] = next_state[zero[0]-1][zero[1]]
    ...
    def compute_heuristic(self):
        ...
        for i in range(3):
            for j in range(3):
                for k in range(3):
                    for m in range(3):
                        if self.state[i][j] == self.goal[k][m] and self.state[i][j] != 0:
                            total += manhattan((i, j), (k, m))
        return total
